...
本文档基于当前源码编写，面向 GitHub Release 发布的二进制版本。
...
- [katana：Web 爬虫（仅 full 版）](#katanaweb-爬虫仅-full-版)
- [passive：网络空间搜索（仅 full 版）](#passive网络空间搜索仅-full-版)
...
默认 provider 是 `deepseek`，默认模型是 `deepseek-v4-pro`。aiscan 可以从 `--base-url` 自动推断 provider（如 URL 包含 `deepseek.com` 则推断为 `deepseek`）。
...
aiscan agent --provider openai --model gpt-4o -p "发现 Web 服务并检查高风险漏洞" -i 192.168.1.0/24
```
...
aiscan agent --provider deepseek --model deepseek-v4-pro -p "枚举服务并输出风险摘要" -i 10.0.0.0/24
```
...
```text
输入目标 → gogo 端口发现 → spray Web 探测/指纹/插件探测/爬取 → zombie 弱口令 → neutron POC → 可选 AI agent 验证/后处理
```

各扫描阶段通过事件队列串联。例如 gogo 发现 HTTP 服务后，Web 目标自动进入 spray；spray 识别到指纹后，指纹用于 neutron 选择 POC。最后的 AI agent 验证/后处理是可选增强，不是 pipeline 内部 capability；启用 `--verify`、`--sniper` 或 `--deep` 后，scan 命令会路由给 LLM agent，由 agent 执行扫描并按对应 sub-skill 做验证、情报搜索或深度分析。

### quick 和 full 模式

| 模式 | 说明 |
| --- | --- |
| `quick` | **默认模式**。gogo 端口扫描（ports=all）、spray check（含 finger）、spray crawl（depth 2）、弱口令、基于指纹的 POC |
| `full` | 在 quick 基础上增加 spray plugins（common/bak/active）和 spray brute（默认字典探测），crawl depth 保持 2 |

full 版二进制会额外接入 katana：quick 增加 `katana_crawl`，full 增加 `katana_crawl` 和 `katana_deep`。
...
| `--verify` | AI 验证模式：`auto`, `off`, `low`, `medium`, `high`, `critical` | `auto` |
...
scan 命令提供三种 AI 增强模式，需要 LLM provider。启用后，aiscan 会把 scan 命令交给 LLM agent 执行；agent 使用扫描器输出作为证据，再按对应 sub-skill 做验证、情报搜索或深度测试。

| 参数 | 说明 |
| --- | --- |
| `--verify=<level>` | 注入 `scan/verify` sub-skill，要求 agent 验证达到阈值的发现 |
| `--sniper` | 注入 `scan/sniper` sub-skill，对发现的指纹搜索公开 CVE/Exploit |
| `--deep` | 注入 `scan/deep` sub-skill，对发现的 Web 资产和指纹做动态测试 |
...
`scan --verify=<priority>` 启用 agent 验证流程，对达到指定优先级的发现进行 LLM 辅助验证。
...
```bash
aiscan scan -i http://target.example --mode quick --verify=high --provider openai --api-key "$OPENAI_API_KEY" --model gpt-4o
...
| 指定 `--loop` | Loop worker：连接 IOA server，注册节点和 IOA 工具 |
| 无任何输入 | 交互式 CLI（REPL） |
...
aiscan zombie -i ssh://admin@127.0.0.1:22 -p admin123 --concurrency 8
aiscan zombie -I services.txt --top 10 --concurrency 8
```

> 注意 `zombie -p` 是密码参数，不是 agent prompt。`--concurrency` 控制每主机最大连接并发，Telnet/Comware 等设备建议保持 `8`；`-c` 是 CIDR 输入，不是并发。
...
`katana` 集成 projectdiscovery/katana，用于深度 Web 爬取和参数发现。

```bash
aiscan-full katana -u https://target.example -d 3 -jc
aiscan-full katana -u https://target.example -d 2 -silent -jsonl
aiscan-full katana -u https://target.example -f qurl
aiscan-full katana -list urls.txt -d 2 -jc -timeout 60
```

| 参数 | 说明 |
| --- | --- |
| `-u, -list` | 目标 URL 或 URL 文件 |
| `-d` | 爬取深度（默认 3） |
| `-jc` | 启用 JavaScript 爬取 |
| `-timeout` | 超时秒数（默认 120） |
| `-f, -field` | 输出字段：`url`, `path`, `qurl`, `qpath`, `file`, `key`, `value`, `kv`, `dir` 等 |
| `-sf, -store-field` | 存储字段 |
| `-em, -extension-match` | 匹配扩展名 |
| `-ef, -extension-filter` | 过滤扩展名 |
| `-jsonl` | JSON Lines 输出 |
| `-silent` | 静默模式 |
...
`passive` 集成 projectdiscovery/uncover，支持 FOFA / Hunter 等网络空间搜索引擎。

```bash
aiscan-full passive -s fofa 'domain="example.com"'
aiscan-full passive -s hunter 'domain.suffix="example.com"'
aiscan-full passive -s shodan-idb '1.2.3.4'
```

| 参数 | 说明 |
| --- | --- |
| `-s <source>` | 数据源（必填）：`fofa`, `hunter`, `shodan-idb` 等 |

需要在全局参数或配置文件中设置对应的 API key：

| 参数 | 环境变量 | 说明 |
| --- | --- | --- |
| `--fofa-email` | `FOFA_EMAIL` | FOFA 账号邮箱 |
| `--fofa-key` | `FOFA_KEY` | FOFA API key |
| `--hunter-api-key` | `HUNTER_API_KEY` | Hunter API key |
| `--recon-proxy` | — | 搜索引擎代理 |
| `--recon-limit` | — | 单次查询资产上限（0 = 无限） |
...
IOA 是 aiscan 的协作层。`ioa serve` 启动本地 HTTP server 和内存 store；`agent --loop` 连接 server 注册 worker，并让 agent 通过 IOA 工具读写协作消息。

### 启动 IOA server

默认监听 `http://127.0.0.1:8765`。当前 CLI 不暴露数据库路径配置，进程重启后 IOA 消息和节点状态不会保留：
...
`agent --loop` 连接 IOA server，注册节点，进入指定 space，并注册 `ioa_space`、`ioa_send`、`ioa_read` 工具。任务消息需要由 agent 主动读取和判断；`--heartbeat` 可用于定期唤醒 agent 巡检 IOA 上下文。

```bash
# 基本 loop worker
aiscan agent --loop --ioa-url http://127.0.0.1:8765 --space case-1 --provider openai --model gpt-4o
...
`--heartbeat <分钟>` 让 loop worker 每隔 N 分钟先读取当前 SpaceInfo 和最近 50 条 IOA 消息，再把 space、node、初始任务或 targets、节点状态和近期消息上下文作为巡检 prompt 注入 agent inbox。agent 可以执行本地工具，也可以通过 IOA 工具发送协调消息或给其他节点分配 task。
...
```bash
export OPENAI_API_KEY="sk-..."
aiscan agent --provider openai --model gpt-4o -p "检查目标" -i http://target.example
```
