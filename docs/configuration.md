...
cyberhub:
  ...
  statuses: ""        # 可选：active,pending 等资源状态过滤
  proxy: ""           # 扫描器代理
...
aiscan agent --provider deepseek --model deepseek-v4-pro -p "扫描目标" -i 10.0.0.0/24
```
...
当前 `ioa serve` 使用内存 store，配置文件不支持设置 IOA 数据库路径。

---

## 被动信息收集

`aiscan-full passive` 和 full 版 recon 能力通过 uncover SDK 使用 FOFA、Hunter 等数据源。基础版会隐藏相关命令和 skill。
...
| `--hunter-token` | `HUNTER_TOKEN` | Hunter web token，通常不需要 |
| `--hunter-api-key` | `HUNTER_API_KEY` | Hunter API key，推荐 |
| `--recon-proxy` | `RECON_PROXY` | 被动查询出站代理 |
| `--recon-limit` | — | 单次查询资产上限，0 表示使用默认 |
...
其他 uncover source（如 Shodan、Censys）继续使用 uncover 支持的环境变量或 `~/.uncover-config/provider-config.yaml`。
