...
- Read aiscan://skills/scan/verify.md before reporting any vulnerability loot.
...
func BuildSystemPrompt(cfg *PromptConfig) string {
	...
	if cfg.CustomPreamble != "" {
		...
		sb.WriteString(fmt.Sprintf(`You are aiscan's %s analysis agent. Execute the requested scanner command using the bash tool, analyze the results, and provide results.
