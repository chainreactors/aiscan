...
// agentStreamingEnabled keeps the agent/provider path event-streamed by
// default, matching pi's streamSimple-first model. Rendering decisions are
// separate: non-TTY stdout can stay buffered while subscribers still receive
...
func (o *AgentOutput) turnEnd(event agent.Event) {
	...
	if role != "" || contentLen > 0 || toolCalls > 0 || reasoningLen > 0 {
		...
		if event.Message.StopReason != "" {
			stopReason = " stop_reason=" + event.Message.StopReason
		}
		fmt.Fprintf(o.stderr, "%s[debug] [turn %d] assistant role=%s content=%d reasoning=%d tool_calls=%d%s preview=%q%s\n",
