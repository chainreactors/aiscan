...
func TestURLInScope(t *testing.T) {
	tests := []struct {
		...
		{name: "empty scope allows", raw: "https://external.example/login", scope: nil, want: true},
		{name: "wildcard allows", raw: "https://external.example/login", scope: []string{"*"}, want: true},
		{name: "same host allows", raw: "https://app.example/login", scope: []string{"app.example"}, want: true},
		{name: "same host port allows", raw: "http://127.0.0.1:4200/login", scope: []string{"127.0.0.1:4200"}, want: true},
		{name: "different host rejects", raw: "https://github.com/login", scope: []string{"127.0.0.1:4200"}, want: false},
		{name: "relative allows", raw: "/login", scope: []string{"127.0.0.1:4200"}, want: true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlInScope(tt.raw, tt.scope); got != tt.want {
				t.Fatalf("urlInScope(%q, %#v) = %v, want %v", tt.raw, tt.scope, got, tt.want)
			}
		})
	}
}

func TestCheckPoolRedirectHonorsScopeBeforeEnqueue(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	base := newTestBasePool(ctx, cancel)
	base.Config.Scope = []string{"127.0.0.1:4200"}
	base.Config.MaxRedirect = 3
	pool := &CheckPool{BasePool: base}

	bl := newCompareBaseline(302, "/", "")
	bl.UrlString = "http://127.0.0.1:4200/"
	bl.RedirectURL = "https://github.com/login"
	bl.Url, _ = url.Parse(bl.UrlString)
	pool.doRedirect(bl, 0)

	select {
	case u := <-pool.additionCh:
		t.Fatalf("external redirect was enqueued: %#v", u)
	default:
	}
	pool.wg.Wait()

	bl.RedirectURL = "http://127.0.0.1:4200/login"
	pool.doRedirect(bl, 0)
	select {
	case u := <-pool.additionCh:
		if u.path != bl.RedirectURL {
			t.Fatalf("redirect path = %q, want %q", u.path, bl.RedirectURL)
		}
		pool.wg.Done()
	case <-time.After(time.Second):
		t.Fatal("same-scope redirect was not enqueued")
	}
	pool.wg.Wait()
}

func TestBrutePoolRedirectHonorsScopeBeforeEnqueue(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	base := newTestBasePool(ctx, cancel)
	base.Config.Scope = []string{"127.0.0.1:4200"}
	base.Config.MaxRedirect = 3
	pool := &BrutePool{BasePool: base}
	...
	case u := <-pool.additionCh:
		if u.path != "/login" {
			t.Fatalf("redirect path = %q, want /login", u.path)
		}
		pool.wg.Done()
	case <-time.After(time.Second):
		t.Fatal("same-scope redirect was not enqueued")
...
func TestOutputSenderwgTracksBlockedHandlerSend(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	pool := newTestBasePool(ctx, cancel)
	pool.OutputCh = make(chan *baseline.Baseline)

	pool.startHandler(func() {
	...
	go func() {
		pool.OutputSenderwg.Wait()
		close(waitDone)
	}()

	select {
	case <-waitDone:
		t.Fatal("OutputSenderwg finished while handler was still blocked in putToOutput")
	...
	case <-pool.OutputCh:
	case <-time.After(time.Second):
		t.Fatal("putToOutput did not reach OutputCh send")
	}
	pool.Outwg.Done()

	mustFinish(t, time.Second, "OutputSenderwg did not finish after send completed", func() {
		<-pool.handlerDone
		pool.OutputSenderwg.Wait()
	})
}

func TestBrutePoolInitDoneWhenInitResultDroppedAfterCancel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	pool := &BrutePool{
		BasePool: newTestBasePool(ctx, cancel),
	}
	pool.processCh = nil
	pool.initwg.Add(1)
	cancel()

	pool.sendUnitProcess(&Unit{source: parsers.InitIndexSource}, newTestBaseline())

	mustFinish(t, time.Second, "initwg not released after canceled init result drop", func() {
		pool.initwg.Wait()
	})
}
