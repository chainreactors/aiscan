...
func NewBrutePool(ctx context.Context, config *Config) (*BrutePool, error) {
	...
	pool.startHandler(pool.Handler)
	return pool, nil
}
...
func (pool *BrutePool) sendUnitProcess(unit *Unit, bl *baseline.Baseline) {
	if pool.sendProcess(bl) {
		return
	}
...
func (pool *BrutePool) doRedirect(bl *baseline.Baseline, depth int) {
	...
	if strings.HasPrefix(bl.RedirectURL, "http") && !urlInScope(bl.RedirectURL, pool.Scope) {
		return
	}
...
func (pool *BrutePool) doCrawl(bl *baseline.Baseline) {
	...
	pool.launchProducer(func() {
		for _, u := range bl.URLs {
			if strings.HasPrefix(u, "http") && !urlInScope(u, pool.Scope) {
				continue
			}
