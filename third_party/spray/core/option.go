...
func (opt *Option) PrintConfig(r *Runner) string {
	...
	if opt.Filter != "" {
		rows = append(rows, []string{keyStyle.Render("⚙ Filter"), formatValue(opt.Filter)})
	}
	if opt.OutputFile != "" {
	...
	return renderConfigTable(rows, lipgloss.NewStyle().Foreground(lipgloss.Color("99")))
}

func renderConfigTable(rows [][]string, borderStyle lipgloss.Style) string {
	if len(rows) == 0 {
		return ""
	}
	...
	for _, row := range rows {
		for i := 0; i < len(row) && i < len(colWidths); i++ {
			if width := configTableCellWidth(row[i]); width > colWidths[i] {
				colWidths[i] = width
			}
		}
	}
	...
	b.WriteString(configTableBorderLine("╭", "┬", "╮", colWidths, borderStyle))
	for _, row := range rows {
		b.WriteByte('\n')
		b.WriteString(configTableRow(row, colWidths, borderStyle))
	}
	b.WriteByte('\n')
	b.WriteString(configTableBorderLine("╰", "┴", "╯", colWidths, borderStyle))
	return b.String()
...
func configTableBorderLine(left, middle, right string, colWidths []int, style lipgloss.Style) string {
	var b strings.Builder
	...
	for i, width := range colWidths {
		...
		if i == len(colWidths)-1 {
			...
			b.WriteString(style.Render(middle))
		}
	}
	return b.String()
}

func configTableRow(row []string, colWidths []int, style lipgloss.Style) string {
	var b strings.Builder
	...
	for i, width := range colWidths {
		...
		b.WriteByte(' ')
		b.WriteString(cell)
		if pad := width - configTableCellWidth(cell); pad > 0 {
...
func configTableCellWidth(cell string) int {
	if cell == "" {
		return 0
	}
	return lipgloss.Width(cell)
}
