...
{"kind": "asset", "domains": ["..."], "ips": ["..."], "source": "ina/fofa"}
...
- **Split by phase**: one peer does recon (e.g. `ani` + `ina`), the other waits for the asset handoff then runs scanning (`gogo` + `spray` + `neutron`). Lowest coordination overhead, best when phases are sequential.
...
- **Silent duplication**: both peers run `ani` on the same company. A cheap `ioa_read all=true` upfront prevents this.
- **Over-coordination**: 10 messages debating who runs `ani`. Just take it and announce — the partner can read and react in one round-trip.
