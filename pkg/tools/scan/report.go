...
func formatMarkdown(d *collector) string {
	...
	sb.WriteString(fmt.Sprintf("| Web pages | %d |\n", len(d.seenWebProbe)))
	sb.WriteString(fmt.Sprintf("| Discovered URLs | %d |\n", len(d.seenWeb)))
...
func formatScanSummaryLine(d *collector, stats statsSnapshot, color bool) string {
	...
	parts = appendCount(parts, len(d.seenWebProbe), "web", "web")
	...
	parts = append(parts, stats.Duration().Round(time.Second).String())
